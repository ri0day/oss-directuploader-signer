...
#callback_url = "http://10.73.64.56:9097";
# 用户上传文件时指定的前缀。
...
def get_token():
    #获取metadata里面的credential
    r = requests.get(f'http://100.100.100.200/latest/meta-data/ram/security-credentials/{rolename}').json()
    access_key_id = r.get("AccessKeyId")
    access_key_secret = r.get("AccessKeySecret")
    sec_token = r.get("SecurityToken")
    ...
    token_dict['x-oss-security-token'] = sec_token
